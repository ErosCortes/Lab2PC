...
def run():
    ...
    avg_resp = logs.groupby('id_server')['response_time_ms'].mean().rename('avg_resp').reset_index()
    merged = avg_resp.merge(hosts[['id', 'hostname']], left_on='id_server', right_on='id', how='left')
    ...
    merged2 = logs.merge(hosts[['id', 'country']], left_on='id_server', right_on='id', how='left')
    pct_fail = merged2.groupby(['id_server', 'country'])['failed'].mean().rename('pct_failed').reset_index()
    ...
    hours = maint.groupby('id_server')['duration_min'].sum().rename('total_min').reset_index()
